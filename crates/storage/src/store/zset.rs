...
use crate::memory::DataCow as Data;
use crate::store::HybridStore;
use crate::traits::{ApplyContext, StoreError, StoreResult, ZSetStore};
use bytes::Bytes;
...
impl ZSetStore for HybridStore {
    fn zadd(
        ...
        let zset = match store_guard.memory_mut().get_mut(key) {
            ...
            None => {
                ...
                store_guard
                    .memory_mut()
                    .insert(key.to_vec(), Data::ZSet(new_zset));
    ...
    fn zscore(&self, key: &[u8], member: &[u8]) -> StoreResult<Option<f64>> {
        ...
        match store_guard.memory().get(key) {
            Some(Data::ZSet(zset)) => Ok(zset.get_score(member)),
    ...
    fn zrank(&self, key: &[u8], member: &[u8]) -> StoreResult<Option<usize>> {
        ...
        match store_guard.memory().get(key) {
            Some(Data::ZSet(zset)) => Ok(zset.rank(member)),
            Some(_) => Err(StoreError::WrongType),
            None => Ok(None),
        }
    }
    ...
    fn zrevrank(&self, key: &[u8], member: &[u8]) -> StoreResult<Option<usize>> {
        ...
        match store_guard.memory().get(key) {
            Some(Data::ZSet(zset)) => Ok(zset.rev_rank(member)),
            Some(_) => Err(StoreError::WrongType),
            None => Ok(None),
        }
    }
    ...
    fn zcard(&self, key: &[u8]) -> StoreResult<usize> {
        ...
        match store_guard.memory().get(key) {
            Some(Data::ZSet(zset)) => Ok(zset.len()),
            Some(_) => Err(StoreError::WrongType),
            None => Ok(0),
        }
    }
